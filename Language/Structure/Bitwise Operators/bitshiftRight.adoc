...
----
Esse comportamento, chamado extensão do sinal, é frequentemente indesejável. Em vez disso, você pode querer que zeros sejam deslocados da esquerda. Acontece que as regras do deslocamento à direita são diferentes para expressões `unsigned int`, então você pode usar casting para impedir que uns sejam copiados da esquerda:
